...
interface Resources extends ClientBundle {
	...
	/**
	 * The CSS resource that "fixes" the SyntaxHighlighter to render correctly in GWT.
	 * 
	 ...
	 * {@link com.alexgorbatchev.syntaxhighlighter.compiler.ThemeGenerator ThemeGenerator}.
	 * 
	...
	interface Brushes extends ClientBundle {
		...
		@ClassType(Wais.class)
		GwtCreateResource<Brush> wais();
		
		@Brush.Source(Paths.BRUSHES + "shBrushWais.js")
		interface Wais extends Brush {
		}
		
		@ClassType(Bson.class)
		GwtCreateResource<Brush> bson();
		
		@Brush.Source(Paths.BRUSHES + "shBrushBson.js")
		interface Bson extends Brush {
	...
	static class Init implements Resources {
		...
		/**
		 * Used in the {@code <style>} and {@code <script>} tags for the resources.
		 */
		...
		/**
		 * Constructor that injects the {@link #coreStyle()}, {@link #GWTFix()}, and {@link #theme()} CSS stylesheets,
		 * and the {@link #coreScript()} JavaScript script into the GWT app.
		 */
		Init() {
			if (!initialized) {
				...
				log.finest("Injecting CSS Resource for theme \"" + theme().getClass().getName() + "\".");
				...
				ScriptElement element = document.createScriptElement(coreScript().getText());
