...
/**
 * Generates the subclass for the {@link com.alexgorbatchev.syntaxhighlighter.client.Theme Theme} class that is defined
 * according to the syntaxhighlighter.theme property in the {@literal <Module>.gwt.xml} file.
 * 
...
public class ThemeGenerator extends BaseGenerator {
	...
	/**
	 * The list of imports necessary for the Theme subclasses that are generated.
	 */
	private static final String[] IMPORTS = { "com.google.gwt.core.client.GWT", "com.google.gwt.resources.client.ClientBundle",
					"com.google.gwt.resources.client.ClientBundle.Source", "com.google.gwt.resources.client.CssResource" };
	/**
	...
	/**
	 * The list of names for the standard set of themes that come packaged with SyntaxHighlighter.
	 */
	private static final String[] STANDARD_THEMES = { "Default", "Django", "Eclipse", "Emacs", "FadeToGrey", "MDUltra", "Midnight", "RDark" };
	
	@Override
	protected void generate(SourceWriter writer) throws UnableToCompleteException {
		String source = buildPath(getScript());
		
		log(TreeLogger.DEBUG, "Setting Theme CSS Source to \"" + source + "\"");
	...
	/**
	 * Validates that the Generator can find the correct CSS resource for the generated Theme.
	 * 
	...
	private String getScript() throws UnableToCompleteException {
		...
		try {
			...
			if (isStandardTheme(theme)) {
				log(TreeLogger.DEBUG, "Using Standard Theme: " + theme);
				script = String.format("shTheme%s", theme);
			}
			else {
				log(TreeLogger.DEBUG, "Using Extended Theme: " + theme);
				script = theme;
			}
			script += ".css";
		}
		...
		if (scriptPath != null) {
			log(TreeLogger.DEBUG, "Theme CSS File: " + scriptPath + script);
		}
		else {
