...
/**
 * Abstract base class for both the {@link BrushGenerator} and {@link ThemeGenerator} classes.
 * 
...
abstract class BaseGenerator extends Generator {
	...
	public final String generate(TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException {
		...
		if (writer != null) {
			...
			log(TreeLogger.DEBUG, "Generated class: " + getQualifiedName());
		}
		
		log(TreeLogger.DEBUG, "Replacing class \"" + type.getQualifiedSourceName() + "\" with \"" + getQualifiedName() + "\".");
	 ...
	 * 
	 * @param writer
	 *            The {@link SourceWriter} that is used to write the Java code with.
	 * 
	...
	/**
	 * 
	 * @return the fully-qualified name of the superclass for the Java class that is generated.
	 */
	 ...
	 */
	protected final SelectionProperty getSelectionProperty(String propertyName) throws BadPropertyValueException {
	 ...
	 * @throws BadPropertyValueException
	 */
	protected final ConfigurationProperty getConfigurationProperty(String propertyName) throws BadPropertyValueException {
	...
	/**
	 * 
	 * @return Array containing all {@link JClassType JClassTypes} that are defined in the {@link #typeOracle}.
	 */
	 ...
	 */
	protected final JClassType getType(String pkgName, String topLevelTypeSimpleName) throws NotFoundException {
	...
	/**
	 * 
	 * @return Array containing all {@link JPackage JPackages} from the {@link #typeOracle}.
	 */
	 ...
	 * @throws NotFoundException
	 */
	protected final JPackage getPackage(String pkgName) throws NotFoundException {
	...
	/**
	 * if (path.contains(".svn")) continue; Added by WMillman
	 * 
	 * @param resource
	 ...
	 *            The path to split.
	 * 
	 * @return {@literal Queue<String>} containing each folder in order defined in the given path.
	 */
	 ...
	 * 
	 * @return The full path for the given resource relative to the new Java class that was generated.
	 */
	protected final String buildPath(String resource) {
		log(TreeLogger.DEBUG, "Building path to resource: " + resource);
		return buildPath(splitPath(getPath(resource)), 0, splitPath(toPath(getPackageName())), 0) + resource;
	}
	
	/**
	 * Recursive method that constructs the complete path to a given resource relative to the new Java class that was
	 * generated.
	 * 
	 * @param origPath
	 *            Original path to the resource based on the original type passed to the Generator.
	 * @param origCnt
	 *            number of folders that have been traversed from the original path.
	 * @param newPath
	 ...
	 */
	private String buildPath(Queue<String> origPath, int origCnt, Queue<String> newPath, int newCnt) {
		if (origPath.isEmpty() && newPath.isEmpty()) {
			...
			if (newPath.poll().trim().length() > 0) {
				return FPAR + FSEP + buildPath(origPath, origCnt, newPath, ++newCnt);
			}
			else {
				return buildPath(origPath, origCnt, newPath, ++newCnt);
			}
	 ...
	 * @throws UnableToCompleteException
	 */
	protected final SourceWriter getSourceWriter() throws UnableToCompleteException {
		ClassSourceFileComposerFactory composer = new ClassSourceFileComposerFactory(getPackageName(), getName());
		...
		PrintWriter printWriter = ctx.tryCreate(log, getPackageName(), getName());
		
		if (printWriter == null) {
			log(TreeLogger.DEBUG, "Class \"" + getName() + "\" already generated.");
			return null;
		}
		else {
